...
| F-04 | **Critical** | Service Bootstrap | Ollama models (`qwen3:1.7b`, `nomic-embed-text`, `qwen3:0.6b`) are documented as required but there is no pre-pull mechanism. The Ollama container will start empty, causing `isli-keeper` to fail on first inference. | `09-tech-stack.md` lines 36–38 list models; `docker-compose.yml` skeleton (line 180–184) has no `entrypoint` script or init container for `ollama pull`. | Add an Ollama init container or a startup script that runs `ollama pull` for each required model before declaring the service healthy. |
...
| F-07 | **High** | Observability | No health check, readiness probe, or liveness probe is defined for any service in the documented compose file or elsewhere. | `09-tech-stack.md` lines 165–202: no `healthcheck` blocks; no `/health` or `/ready` endpoints mentioned in `01-architecture.md`. | Add Docker `healthcheck` entries to every service. Expose `/health`, `/ready`, and `/live` endpoints in `isli-core` and `isli-keeper` that check downstream dependencies. |
