...
| **F-ARCH-02** | **DONE** | **Gap** | **RESOLVED (2026-06-01)**: Implemented durable outbox pattern via `JournalWorker` and `MemoryWorker`. These background processes use PostgreSQL cursors (`journal_updated_at`, `last_memory_extracted_at`) to ensure memory extraction is retried if the Keeper is unavailable, eliminating data loss risks during restarts. | `isli_core/jobs/journal_worker.py`, `isli_core/jobs/memory_worker.py` | Implementation verified. The fire-and-forget pattern has been replaced with reliable polling. |
...
- **F-ARCH-02** (Keeper restart) overlaps with the Memory domain (`03-memory.md`, Tier 2 persistence) and Keeper domain (`02-keeper.md`). **RESOLVED**: A durable outbox for Keeper writes has been implemented via `JournalWorker` and `MemoryWorker`.
...
Out of 11 findings, **4 are Critical** and relate to production-stopping risks: hardcoded service discovery (F-ARCH-01), missing schema registry (F-ARCH-03), task state race conditions (F-ARCH-05), and broken Docker networking (F-ARCH-10). (Note: F-ARCH-02 has been resolved). The architecture is well-suited for a local developer workstation but lacks the resiliency primitives required for a production multi-agent system. Addressing the remaining Critical gaps before adding new agent types or scaling beyond a single machine will prevent the specification-level failures that account for 41.77% of MAST taxonomy incidents.
