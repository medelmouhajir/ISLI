...
class AgentRunner:
    ...
    def _pii_mesh_enabled(self) -> bool:
        return (self.config.config or {}).get("pii_mesh_enabled", False)

    @property
    def _pii_use_slm(self) -> bool:
        return (self.config.config or {}).get("pii_use_slm", True)
    ...
    def _truncate_tool_results_to_cap(self, messages: list[dict], max_tokens: int = 1000) -> list[dict]:
        ...
        if not self.config.turn_token_cap:
            return messages
        ...
        if total_estimated <= self.config.turn_token_cap:
            return messages

        # How much we need to trim
        overage = total_estimated - self.config.turn_token_cap
        ...
        if truncated_count > 0:
            ...
            logger.warning(
                ...
                cap=self.config.turn_token_cap,
    ...
    def _apply_auth_to_kwargs(self, completion_kwargs: dict[str, Any]) -> None:
        ...
        if self.config.api_key:
            ...
            normalized = _normalize_provider(self.config.model_provider or "")
            env_var = provider_env_map.get(normalized)
            if env_var:
                    ...
                    try:
                        async for message in websocket:
                            ...
                            if event["type"] in ("task:created", "task:updated", "task:moved"):
                                ...
                                if token_map:
                                    session_id = payload.get("session_id")
                                    self._pii_client.cache_token_map(session_id, token_map)
    ...
    async def _prepare_llm_payload(
        ...
        if not self._pii_mesh_enabled:
            return system_prompt, messages

        combined = system_prompt + "\n".join(m.get("content", "") for m in messages)
        if not self._pii_client.regex_hits(combined) and not self._pii_client.session_has_tokens(session_id):
            return system_prompt, messages

        try:
            prep = await self._pii_client.session_prep(
                session_id=session_id,
                agent_id=self.config.id,
                messages=messages,
                context_summary=system_prompt,
                mode="pii_only",
                use_slm=self._pii_use_slm,
            )
            self._pii_client.cache_token_map(session_id, prep.get("token_map", {}))
    ...
    def _post_process_response(self, text: str, session_id: str) -> str:
        ...
        if not self._pii_mesh_enabled:
            return text
        return self._pii_client.rehydrate_local(text, session_id)
    ...
    async def _execute_task(self, task_data: dict):
        ...
        try:
            ...
            stream_id = task_data.get("session_id") or task_id
            system_prompt, messages = await self._prepare_llm_payload(system_prompt, messages, stream_id)
                ...
                for tool_call in tool_calls:
                    ...
                    if isinstance(result, str) and (result.startswith("Error:") or result.startswith("Validation Error:")):
                        ...
                        if consecutive_tool_failures >= MAX_CONSECUTIVE_TOOL_FAILURES:
                            logger.error("runner.tool_failures_exceeded", task_id=task_id, tool=tool_name)
                            await self.client.complete_task(
                                task_id,
    ...
    async def _execute_session_message(self, payload: dict):
        ...
        try:
            ...
            # PII Mesh: anonymize before LLM
            system_prompt, messages = await self._prepare_llm_payload(
            ...
            while True:
                ...
                if turn_number > MAX_LLM_TURNS:
                    ...
                    final_text = f"I've encountered an issue completing this request (exceeded maximum turns). Let's pause here."
                    await self._emit_stream_event(
                        session_id,
                        "token_delta",
                        {"text": final_text},
                    ...
                    await self.client.reply_to_session(
                        session_id, final_text, components=list(self._pending_components)
                    )
                    return
                logger.info(
                ...
                for tool_call in tool_calls:
                    ...
                    if isinstance(result, str) and (result.startswith("Error:") or result.startswith("Validation Error:")):
                        consecutive_tool_failures += 1
                        if consecutive_tool_failures >= MAX_CONSECUTIVE_TOOL_FAILURES:
                            logger.error("runner.tool_failures_exceeded", session_id=session_id, tool=tool_name)
                            final_text = f"I'm repeatedly failing to execute a required action (tool: {tool_name}). Let's stop here so you can review."
                            await self._emit_stream_event(session_id, "token_delta", {"text": final_text})
