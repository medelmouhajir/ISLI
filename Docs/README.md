...
| 22 | Skill Metadata Updates | Complete (2026-06-03) — **`update-skill`** endpoint in `isli-skills` (`POST /update`) and SDK tool (`update_skill`) allowing agents to modify existing skill metadata without triggering review; preserves `usage_count` and `created_at` |
