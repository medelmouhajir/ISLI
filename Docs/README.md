...
| 22 | Skill Smith USR Upgrade | Complete (2026-06-18) — `test-skill`, `register-skill`, `update-skill` tools scaffold full USR microservices in the agent workspace; Core owns the workspace-to-USR bridge with path containment, per-agent ownership (`owner_agent_id`), git history preservation, and blue/green updates; legacy AST endpoints in `isli-skills` return `410` |
| 23 | Security Hardening (B2B) | Complete (2026-06-03) — **5 production-critical auth bugs fixed** (empty-JWT god-mode removed, dev-mode auth bypasses closed); **Network segmentation** (`isli-public` / `isli-mesh` / `isli-data` with `internal: true` on data layer); **Docker Compose secret management** (`/run/secrets/` file injection with `_read_secret_file` pydantic validators); **Consolidated service discovery** (~25 `SKILL_*_URL` env vars → 4 upstreams + `ServiceDiscovery` utility); **WebSocket token moved to `Authorization` header** (out of query string); **Pydantic v2 compatibility** (renamed `_auth`/`_admin` → `auth`/`admin` across all services) |
| 24 | Context Injection Refactor | Complete (2026-06-04) — **Unified `ContextWorker`** reading from Redis Stream (`XREADGROUP`) replacing dual DB-polling workers; **three-tier caching** (`ctx:full:{agent_id}:{turn_hash}`) caching fully assembled context strings with event-driven invalidation; **zero HTTP on task path** — `context_summary` delivered inline via `task:updated` WebSocket payload; **complexity delta re-route trigger** (`|Δ| > 6`) for session model switching; **event-driven cache invalidation** on agent config update and journal flush; **DLQ** for messages stuck in PEL after 3 reclaim attempts |
| 25 | Context Safety (Output Caps) | Complete (2026-06-07) — **Hard output caps** for `file-read`, `db-query`, and `git-log` skills; 16k char cap for files; 50 row/500 char cell cap for DB with `max_rows+1` optimization; 12k char cap for git logs; pagination hints and explicit `truncated` observability flags. |
| 26 | Shell Execution (Sandbox) | Complete (2026-06-07) — **`shell-exec` skill** in `isli-skills` using ephemeral Docker containers; network isolation (`--network none`); dropped privileges (`--cap-drop ALL`); read-only rootfs; resource limits (CPU/Mem); output truncation (64KB); SDK `shell_exec()` tool. |
| 27 | Skill Store One-Click Installer | Complete (2026-06-12) — **`POST /v1/skills/install-and-enable`** chains clone→build→run→probe in one call; `last_probe_status`/`last_probe_result` DB columns for real-time observability; `skill:enabled` WebSocket broadcast triggers agent SDK `_sync_config()`; startup resurrection hook re-enables active skills on boot; Board UI shows live status badges (Running/Building/Build Failed) with **Start**/**Stop** toggle, Retry Enable, and Uninstall; `isli skill` CLI sub-app with `install`, `enable`, `disable`, `uninstall`, `list` commands. |
| 28 | Skill Versioning & Updates | Complete (2026-06-13) — Full semver lifecycle for GitHub-sourced skills: `check_update` via `git ls-remote`, blue/green container swap with 30s health probe, `update`/`rollback` endpoints, Redis distributed lock prevents races, `SkillUpdateWorker` auto-applies `update_policy='auto'` skills hourly with `last_checked_at` gating, `skill:updated` WebSocket event triggers agent SDK drain-and-swap tool reload between turns, Board UI shows version badges, changelog preview, update policy dropdown, and "Check All for Updates". |
